...
builder.Services.AddDbContext<JornadaMilhasContext>((options) => {
    options
        ...
        .UseSqlServer(builder.Configuration["ConnectionString:DefaultConnection"], sqlOptions =>
                        sqlOptions.EnableRetryOnFailure(
                            maxRetryCount: 5,  // Número de tentativas de reconexão
                            maxRetryDelay: TimeSpan.FromSeconds(10),  // Tempo máximo entre tentativas
                            errorNumbersToAdd: null) // Tipos de erros para tentar novamente
                  );  
...
async Task ConfigureDefaultUser(IServiceProvider serviceProvider)
{
    using (var scope = serviceProvider.CreateScope())
    {
        ...
        var context = scope.ServiceProvider.GetService<JornadaMilhasContext>(); // Seu DbContext para acesso ao banco de dados
...
async Task AdicionaRegistrosTabela(IServiceProvider serviceProvider)
{
    using (var scope = serviceProvider.CreateScope())
    {
        ...
        {
            if (!context!.OfertasViagem.Any() && !context.Rota.Any())
            { 
